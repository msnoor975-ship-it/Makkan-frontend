...
function CustomerForm({ customer, isEdit }) {
  ...
  const [imagePreview, setImagePreview] = useState(customer?.imageUrl || '')
  ...
  const mutation = useMutation({
    mutationFn: async (data) => {
      if (isEdit) {
        const response = await client.put(`/api/customers/${customer.id}`, data)
  ...
  return (
    <div style={{ 
      ...
      }}>{isEdit ? 'Edit Customer' : 'Add Customer'}</h1>
